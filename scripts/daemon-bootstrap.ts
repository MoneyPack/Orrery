...
// Wire version 2 replaces the v1 Ed25519 `approvalPublicKey` (PEM) with a 32-byte base64url
// `approvalKey` used as a pinned HMAC capability key; the fingerprint is now sha256 over the raw
// key bytes. This is pre-release software and the pipe never crosses machines or persisted
// versions, so v1 frames are rejected outright instead of being negotiated.
const FIELDS = ["type", "version", "handoffNonce", "parentPid", "childPid", "instanceId", "challenge", "approvalKey", "approvalKeyFingerprint"];
...
export interface PromotionBootstrapFrame extends BootstrapBinding { type: "promotion_bootstrap"; version: 2; approvalKey: string; approvalKeyFingerprint: string }
export interface BootstrapChallenge { type: "promotion_bootstrap_challenge"; version: 2; parentPid: number; childPid: number; instanceId: string; challenge: string }

export function createBootstrapFrame(input: BootstrapBinding & { approvalKey: string }): PromotionBootstrapFrame {
  return { type: "promotion_bootstrap", version: 2, ...input, approvalKeyFingerprint: approvalKeyFingerprint(input.approvalKey) };
}
...
export async function readBootstrapFrame(stream: Readable, expected: BootstrapBinding, timeoutMs = BOOTSTRAP_TIMEOUT_MS): Promise<PromotionBootstrapFrame> {
  ...
  if (approvalKeyFingerprint(value.approvalKey) !== value.approvalKeyFingerprint) throw new Error("Managed daemon bootstrap key fingerprint is invalid.");
  return value;
}
...
export async function readBootstrapChallenge(stream: Readable, timeoutMs = BOOTSTRAP_TIMEOUT_MS): Promise<BootstrapChallenge> {
  ...
  if (Object.keys(item).length !== 6 || item.type !== "promotion_bootstrap_challenge" || item.version !== 2 || !validPid(item.parentPid) || !validPid(item.childPid) || !ID.test(String(item.instanceId)) || !HEX64.test(String(item.challenge))) throw new Error("Managed daemon bootstrap challenge is malformed.");
...
function isApprovalKey(value: unknown): value is string { return typeof value === "string" && /^[A-Za-z0-9_-]{43}$/.test(value); }

export type BootstrapDuplex = Duplex;

export function completeParentBootstrap(child: ChildProcess, handoffNonce: string, approvalKey: string, parentPid = process.pid): Promise<{ instanceId: string; approvalKeyFingerprint: string }> {
  ...
  return readBootstrapChallenge(challengePipe).then((challenge) => {
    ...
    const frame = createBootstrapFrame({ handoffNonce, parentPid, childPid: child.pid!, instanceId: challenge.instanceId, challenge: challenge.challenge, approvalKey });
