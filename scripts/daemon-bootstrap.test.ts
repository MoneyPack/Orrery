...
describe("managed daemon inherited bootstrap", () => {
  it("round-trips one bounded exact frame with matching key fingerprint", async () => {
    ...
    const frame = createBootstrapFrame({ ...binding, approvalKey });
  ...
  it.each([
    ...
    const pipe = new PassThrough();
    writeBootstrapFrame(pipe, createBootstrapFrame({ ...binding, [field]: value, approvalKey }));
  ...
  it("rejects malformed, duplicate, oversized, and fingerprint-mismatched frames", async () => {
    ...
    expect(frame.approvalKeyFingerprint).toBe(approvalKeyFingerprint(approvalKey));
  });
  ...
  it("rejects trailing bytes after the single bootstrap challenge", async () => {
    const pipe = new PassThrough();
    const challenge = { type: "promotion_bootstrap_challenge", version: 2, parentPid: 100, childPid: 200, instanceId: "instance-1", challenge: "a".repeat(64) } as const;
